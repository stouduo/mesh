package com.stouduo.mesh.registry.etcd;
...
public class EtcdRegistry extends BaseRegistry implements IRegistry {
    ...
    private void watch(Client client) {
        final Watcher watcher = client.getWatchClient().watch(
                ByteSequence.fromString(registryKey),
                WatchOption.newBuilder().withRevision(revsion).build()
        );
                        ...
                        while (keepListen) {
                            ...
                            for (WatchEvent event : response.getEvents()) {
                                switch (event.getEventType()) {
                                    case PUT:
                                        providers.put(findKey, findEndPoints(findKey));
                                        break;
                                    case DELETE:
                                        providers.put(findKey, findEndPoints(findKey));
                                        keepListen = false;
    ...
    private List<Endpoint> findEndPoints(String findKey) throws Exception {
    ...
    public List<Endpoint> find(String serviceName) throws Exception {
        ...
        endpoints = findEndPoints(findKey);
